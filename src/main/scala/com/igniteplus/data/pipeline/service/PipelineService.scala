...
import com.igniteplus.data.pipeline.constants.ApplicationConstants.{CLICKSTREAM_COLUMNS_CHECK_NULL, COL_TIMESTAMP, DEPARTMENT_NAME, CSV_FILE_TYPE, INPUT_LOCATION_CLICKSTREAM, INPUT_LOCATION_ITEM, INPUT_NULL_CLICKSTREAM_DATA, INPUT_NULL_ITEM_DATA, ITEM_COLUMNS_CHECK_NULL, NIL_VALUE, REDIRECTION_SOURCE, SEQ_CLICKSTREAM_PRIMARY_KEYS, SEQ_ITEM_PRIMARY_KEYS, TIMESTAMP_FORMAT, TO_TIMESTAMP, spark}
...
object PipelineService
{
  def pipelineService () : Unit  =
  {
    ...
    val notNullClickstreamDf:DataFrame=removeNull(validatedClickstremDf,CLICKSTREAM_COLUMNS_CHECK_NULL,INPUT_NULL_CLICKSTREAM_DATA,CSV_FILE_TYPE)
    val notNullItemDf:DataFrame=removeNull(itemDf,ITEM_COLUMNS_CHECK_NULL,INPUT_NULL_ITEM_DATA,CSV_FILE_TYPE)
